...
void man()
{
	...
	printf("  -g                   (Generates a dfu file from the hex file ) \n");
	printf("  Usage Example: DfuSeCommand.exe -g path/file.hex -out path/file.dfu -Vid 0483 -Pid DF11 \n");
...
bool Is_Option(char* option)
{
	...
	else if (strcmp(option,"-g")==0) return true;
    else return false;
}
	...
	else
	{
		...
		while (arg_index < argc)
		{
			...
			else if (strcmp(argv[arg_index], "-g") == 0)
			{
				...
				if (arg_index + 7 > argc)
				{
					printf("Error: Not enough parameters, refer to the following example:\n");
					printf("  Usage Example: DfuSeCommand.exe -g path/file.hex -out path/file.dfu -Vid 0483 -Pid DF11 \n");
					return 1;
				}
				//we have enough params
				arg_index++;
				CString hexFilePath = argv[arg_index];

				arg_index++;
				if (strcmp(argv[arg_index], "-out") != 0)
				{
					printf("Error: -out argument missing:\n");
					printf("  Usage Example: DfuSeCommand.exe -g path/file.hex -out path/file.dfu -Vid 0483 -Pid DF11 \n");
					return 1;
				}

				arg_index++;
				CString dfuFilePath = argv[arg_index];

				arg_index++;
				if (strcmp(argv[arg_index], "-Vid") != 0)
				{
					printf("Error: -Vid argument missing:\n");
					printf("  Usage Example: DfuSeCommand.exe -g path/file.hex -out path/file.dfu -Vid 0483 -Pid DF11 \n");
					return 1;
				}
				...
				{
					printf("Error: null value not accepted in arguments:\n");
					printf("  Usage Example: DfuSeCommand.exe -g path/file.hex -out path/file.dfu -Vid 0483 -Pid DF11 \n");
					return 1;
				}
				Vid = (WORD)strtoul(argv[arg_index], &dummy, 16);
				if (Vid == 0)
				{
					printf("Error: Vid value cannot be zero or null:\n");
					printf("  Usage Example: DfuSeCommand.exe -g path/file.hex -out path/file.dfu -Vid 0483 -Pid DF11 \n");
					return 1;
				}
				...
				{
					printf("Error: -Pid argument missing:\n");
					printf("  Usage Example: DfuSeCommand.exe -g path/file.hex -out path/file.dfu -Vid 0483 -Pid DF11 \n");
					return 1;
				}
				...
				Pid = (WORD)strtoul(argv[arg_index], &dummy, 16);
				if (Pid == 0)
				{
					printf("Error: Pid value cannot be zero or null:\n");
					printf("  Usage Example: DfuSeCommand.exe -g path/file.hex -out path/file.dfu -Vid 0483 -Pid DF11 \n");
					return 1;
				}
				...
				//generate the image
				if (STDFUFILES_ImageFromFile((LPSTR)(LPCSTR)hexFilePath, &hImage, 0) != STDFUFILES_NOERROR)
				{
					printf("Error: Failed to generate an image of the hex file:\n");
					return 1;
				}

				//create the dfu file now
				if (STDFUFILES_CreateNewDFUFile((LPSTR)(LPCSTR)dfuFilePath, &hFile, Vid, Pid, Bcd) == STDFUFILES_NOERROR)
				{
					if (STDFUFILES_AppendImageToDFUFile(hFile, hImage) == STDFUFILES_NOERROR)
					{
						printf("Dfu file creation successful");
					}
					else
					{
						printf("Dfu file creation failed");
					}
					STDFUFILES_CloseDFUFile(hFile);
				}
